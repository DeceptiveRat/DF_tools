...
def usage():
	...
	print("\t-v: set verbose level to 1; prints current offset of search")
	print("\t-c <int>: set number of entries being parsed. Default 1")

def main():
	try:
		opts, args = getopt.getopt(sys.argv[1:], "hf:r:ide:qbRvc:")
	...
	for _ in range(entry_count):
		...
		while current_offset < INDEX_RECORD_SIZE - 16:
			...
			# search for next entry
			if index_entry == None:
				if not quiet_mode and verbose_level >= 1:
